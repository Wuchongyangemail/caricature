 ...
 * @Description
 * @Author wcy
 * @Date 2022/4/10 下午7:44
 **/
@Slf4j
public class AreaExcelImport {
    /**
     * excel2007及以上版本
     *
     * @param file
     ...
     * @throws IOException
     */
    static List<Area> xlsxImportExcel(MultipartFile file) throws IOException {
        ...
        for (int rowNum = 2; rowNum < xssfSheet.getLastRowNum() + 1; rowNum++) {
            ...
            for (int cellNum = 0; cellNum < xssfRow.getLastCellNum(); cellNum++) {
                ...
                switch (cellNum) {
                    case 0:
                        area.setAreaName(String.valueOf(getXSSFValue(xssCell)));
                        break;
                }
     ...
     * @throws IOException
     */
    static List<Area> xlsImportExcel(MultipartFile file) throws IOException {
        ...
        for (int rowNum = 2; rowNum < sheet.getLastRowNum() + 1; rowNum++) {
            ...
            for (int cellNum = 0; cellNum < row.getLastCellNum(); cellNum++) {
                ...
                switch (cellNum) {
                    case 0:
                        area.setAreaName(String.valueOf(getValue(cell)));
                        break;
                }
                log.info("内容：" + getValue(cell));

            }
            list.add(area);    //将excel每一行的数据封装到user对象,并将user对象添加到list
        }
        return list;
    }

    public static List<Area> importExcel(MultipartFile file) throws IOException {
